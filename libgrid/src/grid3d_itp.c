/*
 * Routines to find eigenfunctions by the imaginary time propagation method in 3D.
 *
 ...
 *
 * gwf            = an array of dimension "states" for storing the resulting eigenfunctions (wf3d **).
...
EXPORT double grid3d_itp_linear(wf3d **gwf, int states, int virtuals, const cgrid3d *potential, double tau, double threshold, int max_iterations, double *rtau, int *riterations) {
  ...
  cgrid3d *sq_grad_pot = 0;
  cgrid3d *workspace  = 0;
  cgrid3d *workspace2 = 0;
  ...
  gwf_short = (wf3d **) malloc(states * sizeof(wf3d *));
  for(i = 0; i < states; i++) {
    gwf_short[i] = grid3d_wf_alloc(gwf[i]->grid->nx, gwf[i]->grid->ny, gwf[i]->grid->nz, gwf[i]->grid->step,
				 gwf[i]->mass, gwf[i]->boundary, gwf[i]->propagator);
    
    if (!gwf_short[i]) {
      fprintf(stderr, "libgrid: Error in grid3d_itp_linear(). Could not allocate memory for workspaces.\n");
      abort();
    }
  }
  
  /* allocate grids for |grad pot|^2 and workspaces */
  sq_grad_pot = cgrid3d_alloc(potential->nx, potential->ny, potential->nz, potential->step,
			     potential->value_outside, potential->outside_params_ptr);
  workspace = cgrid3d_alloc(potential->nx, potential->ny, potential->nz, potential->step,
			   potential->value_outside, potential->outside_params_ptr);
  workspace2 = cgrid3d_alloc(potential->nx, potential->ny, potential->nz, potential->step,
			    potential->value_outside, potential->outside_params_ptr);
  ...
  /* copy wave functions */
  for(i = 0; i < states; i++)
    cgrid3d_copy(gwf_short[i]->grid, gwf_long[i]->grid);
  ...
  for(l = 0; l < max_iterations; l++) {
    ...
    time_step = -I * tau;
    for(i = 0; i < states; i++)
      grid3d_wf_propagate(gwf_long[i], potential, sq_grad_pot, time_step, workspace);
    grid3d_wf_diagonalize(gwf_long, states);
    
    for(i = 0; i < states; i++)
      energy_long[i] = grid3d_wf_energy_and_error(gwf_long[i], potential, workspace, &error_long[i]); 
    ...
    for(i = 0; i < states; i++)
      grid3d_wf_propagate(gwf_short[i], potential, sq_grad_pot, time_step, workspace);
    
    grid3d_wf_diagonalize(gwf_short, states);
    
    for( i = 0; i < states; i++ ) 
      energy_short[i] = grid3d_wf_energy_and_error(gwf_short[i], potential, workspace, &error_short[i]);
    ...
    /* else use short time step and corresponing wave function */
      ...
      for(i = 0; i < states; i++)
        cgrid3d_copy(gwf_long[i]->grid, gwf_short[i]->grid);
 ...
 * states               = number of states requested (int).
 * virtuals             = number of virtual states for orthogonalization (int).
 * calculate_potentials = nonlinear potential, which takes the current grid etc. as argument (void (*)(cgrid3d **, void *, wf3d **, int)).
...
EXPORT double grid3d_itp_nonlinear(wf3d **gwf, int states, int virtuals, void (*calculate_potentials)(cgrid3d **potential, void *arg, wf3d **gwf, int states), void *arg, double tau, double threshold, int max_iterations, double *rtau, int *riterations) {
  ...
  sq_grad_pot = (cgrid3d **) malloc(states * sizeof(cgrid3d *));
  
  for(i = 0; i < states; i++) {
    gwf_short[i] = grid3d_wf_alloc(gwf[i]->grid->nx, gwf[i]->grid->ny, gwf[i]->grid->nz, gwf[i]->grid->step,
				   gwf[i]->mass, gwf[i]->boundary, gwf[i]->propagator);
    potential[i] = cgrid3d_alloc(gwf[i]->grid->nx, gwf[i]->grid->ny, gwf[i]->grid->nz, gwf[i]->grid->step,
				gwf[i]->grid->value_outside, gwf[i]->grid->outside_params_ptr);
    sq_grad_pot[i] = cgrid3d_alloc(potential[i]->nx, potential[i]->ny, potential[i]->nz, potential[i]->step,
				  potential[i]->value_outside, potential[i]->outside_params_ptr);
  }
  workspace = cgrid3d_alloc(potential[0]->nx, potential[0]->ny, potential[0]->nz, potential[0]->step,
			   potential[0]->value_outside, potential[0]->outside_params_ptr);
  workspace2 = cgrid3d_alloc(potential[0]->nx, potential[0]->ny, potential[0]->nz, potential[0]->step,
			    potential[0]->value_outside, potential[0]->outside_params_ptr);
  
  for(i = 0; i < states; i++)
    if (!gwf_short[i] || !potential[i] || !sq_grad_pot[i]) {
      fprintf(stderr, "libgrid: Error in grid3d_itp_nonlinear(). Could not allocate memory for workspaces.\n");
      abort();
    }
  ...
  for(l = 0; l < max_iterations; l++) {
    ...
    /* |grad potential|^2 */
    for(i = 0; i < states; i++)
      grid3d_wf_square_of_potential_gradient(sq_grad_pot[i], potential[i], workspace, workspace2);
    
    /* propagate t = - i tau */
    time_step = -I * tau;
    for(i = 0; i < states; i++)
      grid3d_wf_propagate(gwf_long[i], potential[i], sq_grad_pot[i], time_step, workspace);
    
    grid3d_wf_diagonalize(gwf_long, states);
    
    for(i = 0; i < states; i++)
      energy_long[i] = grid3d_wf_energy_and_error(gwf_long[i], potential[i], workspace, &error_long[i]);
    ...
    time_step = -I * cfactor * tau;
    for(i = 0; i < states; i++)
      grid3d_wf_propagate(gwf_short[i], potential[i], sq_grad_pot[i], time_step, workspace);
    
    grid3d_wf_diagonalize(gwf_short, states);
    
    for(i = 0; i < states; i++)
      energy_short[i] = grid3d_wf_energy_and_error(gwf_short[i], potential[i], workspace, &error_short[i]);
  ...
  for(i = 0; i < states; i++) {
    grid3d_wf_free(gwf_short[i]);
    cgrid3d_free(potential[i]);
    cgrid3d_free(sq_grad_pot[i]);
  }
  free(gwf_short);
  free(potential);
  free(sq_grad_pot);
