 ...
 * 
 * grid = 2D grid for which the FFT allocation is to be done (rgrid2d *).
 *
 * No return value.
 *
 */
 ...
 *
 * grid = 2D grid for which the FFT buffers are to be freed (rgrid2d *).
 *
 * No return value.
 *
 */

EXPORT void rgrid2d_fftw_free(rgrid2d *grid) {
 ...
 *
 * grid = 2D grid to be transformed (rgrid2d *).
 *
 * No return value.
 *
 */

EXPORT void rgrid2d_fftw(rgrid2d *grid) {

  fftw_execute(grid->plan);
  memcpy(grid->cint->value, temp2, sizeof(double complex) * grid->cint->nx * grid->cint->ny);
}
...
EXPORT void rgrid2d_fftw_inv(rgrid2d *grid) {

  fftw_execute(grid->iplan);
  memcpy(grid->value, temp, sizeof(double) * grid->nx * grid->ny);
}
